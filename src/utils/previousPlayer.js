...
import { getDummyPrevPlayer } from "./dummyStore";
import { isBackendMode } from "./dataSource";
import { fetchBackendLastSoldPlayer } from "./backendApi";
...
export const fetchPrevPlayer = async () => {
    if (isDummyMode()) {
        return getDummyPrevPlayer();
    }
    if (isBackendMode()) {
        try {
            return await fetchBackendLastSoldPlayer();
        } catch (error) {
