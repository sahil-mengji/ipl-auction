...
import { getDummySoldPlayers } from "./dummyStore";
import { isBackendMode } from "./dataSource";
import { fetchBackendSoldPlayers } from "./backendApi";
...
export const fetcnsoldPlayers = async () => {
    if (isDummyMode()) {
        return getDummySoldPlayers();
    }
    if (isBackendMode()) {
        try {
            return await fetchBackendSoldPlayers();
        } catch (error) {
