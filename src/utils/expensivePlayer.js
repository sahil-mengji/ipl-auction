...
import { getDummyExpensivePlayer } from "./dummyStore";
import { isBackendMode } from "./dataSource";
import { fetchBackendExpensivePlayer } from "./backendApi";
...
export const fetchExpensivePlayer = async () => {
    if (isDummyMode()) {
        return getDummyExpensivePlayer();
    }
    if (isBackendMode()) {
        try {
            return await fetchBackendExpensivePlayer();
        } catch (error) {
